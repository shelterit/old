...
class xs_RestService extends xs_Core {
    ...
    function __construct ( $url = '', $config = null ) {
        ...
        foreach ( $this->config as $idx => $item )
            $this->http->addVar ( 'param:'.$idx, $item ) ;

            // print_r ( $url ) ;
            // print_r ( $config ) ;
            
        /// if ( isset ( $this->config['list'] ) )
            // $this->http->addVar ( 'list', $this->config['list'] ) ;
            // $this->http->postvars = array ( 'list' => serialize ( $this->config['param']['list'] ) ) ;
    }

    function POST () {
        if ( ! $this->http->fetch ( $this->url ) ) {
            echo "RestService: '".$this->url."' ERROR! \n" ;
            print_r ( $this->http->log ) ;
            // die() ;
        }
    }
...
class http {
    ...
    function addVar ( $idx, $var ) {
        $this->postvars[$idx] = serialize($var) ;
    }
    ...
    */
    function fetch($url="", $ttl=0, $name="", $user="", $pwd="", $verb="GET") {
        ...
        Get response header.
        */
        $this->log .= "file handle : ".print_r ( $fh, true )."<br />";

        $this->header = fgets($fh, 1024);
        $this->status = substr($this->header,9,3);
        $this->log .= "got header : ".print_r ( $this->header, true )."<br />";
    ...
    */
    function getFromUrl($url, $user="", $pwd="", $verb="GET") {
        ...
        $this->log .= "getFromUrl() : ".print_r ( $parts, true )."<br />";
        ...
        $f = ((strtolower($protocol) == "https://")?"ssl://":"").$server ;
        $this->log .= "Trying: [$f : $port]<br />\n";
        if (!$sock = fsockopen($f, $port, $errno, $errstr, $this->connect_timeout)) {
            $this->log .= "Could not open connection. Error ".$errno.": ".$errstr."<br />\n";
            return false;
        }

        $this->log .= "getFromUrl() : ".print_r ( $sock, true )."<br />";
        // print_r ( $sock ) ;
        ...
        if (count($this->postvars) > 0) {
            ...
            $this->log .= "getFromUrl() post_string : ".print_r ( $post_string, true )."<br />";
        ...
        $this->log .= "getFromUrl() request : ".print_r ( $request, true )."<br />";
        ...
        $this->log .= "getFromUrl() head : ".print_r ( $this->headers, true )."<br />";
